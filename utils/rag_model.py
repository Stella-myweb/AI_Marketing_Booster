...
class RAGModel:
    ...
    def generate_diagnosis_report(self, answers: Dict[str, str], diagnosis_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        자가진단 결과를 바탕으로 전문적이고 구체적인 전략 가이드를 생성합니다. (1000자 이내, 이모티콘 소제목)
        """
        try:
            ...
            # 프롬프트
            prompt = f"""
            당신은 네이버 스마트 플레이스 최적화 전문가입니다. 아래 진단 결과와 참고 자료를 바탕으로 1000자 이내의 전문적 분석 보고서를 작성하세요. 각 소제목은 이모티콘(예: # 📊, # 🎯, # 💡)으로 구분해 주세요.
            ...
            return {
                ...
                "level": level,
                "summary": report[:1000]  # 1000자 이내로 제한
            }
        except Exception as e:
